Shader "Hidden/SSAO"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            int _SampleCount;
            half _Radius, _Intensity, _Power;

            float4 frag_ssao(v2f i, float4 screenPos : SV_Position) : SV_Target
            {
                ...
                const int numSteps = 4; // Number of steps per ray
                ...
                for (int j = 0; j < _SampleCount; j++)
                {
                    ...
                    // Take multiple steps along the ray
                    for (int step = 1; step <= numSteps; ++step)
                    {
                        float rayFrac = (float)step / numSteps;
                        float3 samplePos = origin + viewspaceRayDir * _Radius * rayFrac * xi.x;
                        ...
                        // Check if the sample point is behind the scene geometry
                        if (sampleDepth > sceneDepth)
                        {
                            half falloff = 1.0h - saturate((sampleDepth - sceneDepth) / _Radius);
                            occlusion += pow(falloff, 2.0);
                            break; // Stop checking this ray once an occluder is found
                        }
                    }
                }

                if (_SampleCount > 0) occlusion /= (half)_SampleCount;
                
                occlusion = 1.0h - occlusion * _Intensity;
